...
def validate_service(service: Any, context: str, errors: list[str]) -> None:
    ...
    if "proxy" in service_map:
        require_mapping(service_map["proxy"], f"{context}.proxy", errors)
...
def validate_common_workload(
    ...
    for index, service in enumerate(services):
        validate_service(service, f"{context}.services[{index}]", errors)

    require_mapping(workload.get("operations", {}), f"{context}.operations", errors)


def validate_ct_workload(
    workload: dict[str, Any], context: str, nodes: set[str], networks: set[str], errors: list[str]
) -> None:
    validate_common_workload(workload, context, nodes, networks, errors)
...
def validate_environment_document(document: dict[str, Any], environment: str) -> list[str]:
    ...
    for name, workload in document.get("cts", {}).items():
        ...
        validate_ct_workload(workload, context, node_names, network_names, errors)
    ...
    for name, workload in document.get("vms", {}).items():
        ...
        validate_vm_workload(workload, context, node_names, network_names, errors)
        vmid = workload.get("vmid")
        if isinstance(vmid, int):
            owner = seen_vmids.get(vmid)
            if owner is not None:
