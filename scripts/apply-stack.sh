...
if [[ "$STACK" == "proxmox-base" ]]; then
  ...
  if ! ensure_non_placeholder "proxmox_api_url" "$proxmox_api_url" \
    ...
    cat >&2 <<'EOF'
proxmox-base requires real Proxmox credentials and bootstrap secrets before terraform apply can run meaningfully.
...
Without valid Proxmox credentials, terraform apply does not validate provider-backed behavior for this stack.
EOF
    exit 1
  fi
fi
...
if [[ -f "$COMMON_VARS" ]]; then
  apply_args+=(-var-file "$(to_runner_path "$COMMON_VARS")")
fi

if [[ -f "$STACK_VARS" ]]; then
  apply_args+=(-var-file "$(to_runner_path "$STACK_VARS")")
fi

run_terraform -chdir="$TF_STACK_DIR" apply "${apply_args[@]}" "$@"
